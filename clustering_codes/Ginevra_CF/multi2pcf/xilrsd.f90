!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!2PCF redshift-space multipoles of the UNIT 2Gpc/h mock
...
Program  main
  ...
  integer, parameter :: Nth=16, Nheader=2, Ns_bins=200, Nmu_bins=120
  ...
  real*8   :: s_bin_l(1:Ns_bins),s_bin_r(1:Ns_bins),s_bin_c(1:Ns_bins), &
              ...
              dvol(1:Ns_bins,1:Nmu_bins),dn(1:Ns_bins,1:Nmu_bins)

  real*8   :: bucket_data_data(1:Ns_bins,1:Nmu_bins,1:Nth),bucket_data_data_quad(1:Ns_bins,1:Nmu_bins,1:Nth), &
       bucket_data_data_hexa(1:Ns_bins,1:Nmu_bins,1:Nth),bucket_random(1:Ns_bins,1:Nmu_bins),bucket_random_quad(1:Ns_bins,1:Nmu_bins), &
       bucket_random_hexa(1:Ns_bins,1:Nmu_bins), &
       bucket_data_data_new(1:Ns_bins,1:Nmu_bins,1:Nth),bucket_data_data_new_final(1:Ns_bins,1:Nmu_bins), &
       bucket_data_data_quad_new(1:Ns_bins,1:Nmu_bins,1:Nth),bucket_data_data_quad_new_final(1:Ns_bins,1:Nmu_bins), &
       bucket_data_data_hexa_new(1:Ns_bins,1:Nmu_bins,1:Nth),bucket_data_data_hexa_new_final(1:Ns_bins,1:Nmu_bins), &
       cf(1:Ns_bins,1:Nmu_bins),cfwedges(1:Ns_bins),mono(1:Ns_bins),quad(1:Ns_bins),hexa(1:Ns_bins), &
       cf_transvwedge(1:Ns_bins), cf_parwedge(1:Ns_bins)

  real*8   :: x_temp,y_temp,z_temp,junk1,junk2,vz_temp,hubble053,hubbleEV

  real*8   :: step_mu,step_s,mu,s,dx,dy,dz,rz
  integer  :: i,j,k,n,io,iom,p,t,ioma,l,m,g,beginning,rate,end
...
hubbleEV=dble(hubble0*sqrt(omegam*(1.+redshift)**3.+omegal))
  ...
  ENDDO
  DO WHILE (io >= 0)
     READ(1,*,IOSTAT=io) x_temp,y_temp,junk1,z_temp
     x(i) = x_temp
     y(i) = y_temp
     z(i) = z_temp
     i = i+1
